...
def genetic_algorithm(rectangles, bin_width, bin_height, population_size, iteration, crossover_rate, mutation_rate): #Genetik Algoritma Fonksiyonu
    ...
    for i in range(iteration): #Belirli bir iterasyon sayısı kadar döngü yapma işlemi
        ...
        self.result_textbox.setPlainText(f'Iterasyon {i + 1} - En iyi fitness: {best_fitness:.2f}')
        self.progress_bar.setValue(i + 1)  # İterasyon sayısını güncelle
        self.progress_bar.setFormat(f'Geçen Süre: {elapsed_time:.2f} saniye')"""
...
class PackingApp(QMainWindow):
    ...
    def run_genetic_algorithm(self):
        ...
        best_chromosome, best_fitness = genetic_algorithm(self.rectangles, bin_width, bin_height, population_size, iteration, crossover_rate, mutation_rate)
